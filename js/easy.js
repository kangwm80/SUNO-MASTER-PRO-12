...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const selections = {
        ...
        situationCategory: '',  // 상황 카테고리 (예: '카페')
        situationText: ''       // 장소/상황 상세 문장
    };
    ...
    if (easySituationSelect) {
        easySituationSelect.addEventListener('change', () => {
            ...
            if (easySituationSelect.value && themeVal) {
                ...
                selections.situationText = selectedOption.textContent.trim().replace(/^[^\s]+\s/, '');
    ...
    function buildFinalPrompt() {
        ...
        const sitInfo = (selections.situationCategory || selections.situationText) ? { category: selections.situationCategory, text: selections.situationText } : undefined;
        const result = generatePrompt(selections.genres, selections.target, selections.place, selections.mood, vocalOpts, sitInfo);
...
`;
    ...
    btnApply.addEventListener('click', () => {
        ...
        // === 강제 저장: 현재 드롭다운에서 선택된 상황 카테고리/장소 상황을 무조건 저장 ===
        const situationThemeEl = document.getElementById('easySituationTheme');
        const situationSelectEl = document.getElementById('easySituationSelect');
        if (situationThemeEl && situationThemeEl.value) {
            selections.situationCategory = situationThemeEl.value;
        }
        if (situationSelectEl && situationSelectEl.value) {
            const opt = situationSelectEl.options[situationSelectEl.selectedIndex];
