...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function buildStoryPool(gen, place, sitText) {
        ...
        const pools = {
            '10대': [
                // ① 현재 감정
                `오늘 이상하게 심장이 두근거렸어. ${subjectCtx}를 생각하다가 갑자기 그 감정이 밀려왔는데, 평소라면 그냥 지나쳤을 것들이 오늘따라 다 다르게 보였어. ${atmo} 이 느낌이 바로 ${feeling}이라는 거구나 싶었어. 이어폰 꽂고 혼자 걸으면서 가슴 어딘가가 꽉 찬 기분, 말로는 설명이 안 되는데 분명히 좋은 감정이었어. 이런 순간이 있다는 것 자체가 지금 내가 살아있다는 증거 같았어.`,
                // ② 과거 회상
                `작년 이맘때가 떠올랐어. 그때도 ${themeCtx}에 대해 비슷한 고민을 했던 것 같은데, 지금의 나랑은 확실히 달라. ${atmo} 감성처럼, 그때는 그냥 무섭기만 했던 것들이 지금은 조금 설레기도 해. 성장이 거창한 게 아니라 이런 작은 감정의 변화에서 온다는 걸 오늘 처음 알았어. ${feeling}처럼, 과거의 내가 지금의 나를 만들었다는 게 새삼 신기하고 고마웠어.`,
                // ③ 관계 중심
                `좋아하는 사람한테 ${subjectCtx} 얘기를 꺼내려다가 그냥 뒀어. 어떻게 말해야 할지 모르겠어서. ${atmo} 분위기 속에서 그 사람이 웃는 걸 보면 ${feeling}이 절로 느껴지는데, 정작 내 마음은 아직 말 못 하고 있어. 말 못 한 마음이 이렇게 쌓이다 보면 언젠가는 노래가 되겠지 싶었어. 그 노래가 완성될 때쯤엔 용기도 같이 생겨 있으면 좋겠어.`,
                // ④ 내면 독백
                `사실 나 요즘 많이 지쳐있어. 학교, 학원, 친구 관계, 부모님 기대까지 다 버겁거든. ${subjectCtx}에 대해 생각하다 보면 잠깐이나마 그 무게가 가벼워지는 것 같아. ${atmo} 감성 속에서 혼자 있을 때만큼은 억지로 웃지 않아도 되는 것 같아서 그게 제일 좋아. 이런 감정을 느끼는 게 약한 게 아니라는 걸 이제는 조금 알 것 같아. ${feeling}처럼, 내 마음도 진짜라는 걸 오늘은 인정해주고 싶었어.`,
                // ⑤ 전환과 다짐
                `오늘 한 가지 결심했어. 더 이상 남들 눈치 보면서 살지 않겠다는 거야. ${subjectCtx}를 생각하면서 ${feeling}이 담긴 이 감정이 나만의 것이라는 걸 처음으로 자랑스럽게 느꼈어. ${atmo} 이 분위기처럼 나는 내 속도대로 살아갈 거야. 아직 어리다고, 몰라서 그렇다고 하는 말들은 이제 흘려보낼게. 지금의 내 감정은 진짜고, 그게 제일 중요해.`
            ],
            '2030세대': [
                // ① 현재 감정
                `번아웃이라는 말이 이렇게 정확하게 맞는 순간이 올 줄 몰랐어. ${subjectCtx}를 생각하다가 문득 멈춰서, 나 지금 뭘 위해 이렇게 달리고 있는 건지 모르겠다는 생각이 들었어. ${atmo} 감성 속에서 그냥 아무것도 하지 않고 가만히 있었는데, 이게 이렇게 사치스럽게 느껴질 줄은 몰랐어. ${feeling}처럼, 충전이 필요하다는 걸 이제는 인정할 수 있어. 내일의 나를 위해 오늘의 나를 쉬게 해주기로 했어.`,
                // ② 과거 회상
                `취업 준비하던 그 시절, ${themeCtx}가 나한테 이런 의미일 줄 몰랐어. 그때는 아무것도 확실하지 않았는데, 솔직히 지금도 별로 확실한 게 없긴 해. 그래도 그때의 내가 포기하지 않고 버텨준 덕분에 지금의 내가 있는 거잖아. ${atmo} 감성처럼, 흔들리면서도 넘어지지 않은 나 자신이 사실 꽤 대단하다고 처음으로 솔직하게 인정해봤어. 잘하고 있어, 진짜로.`,
                // ③ 관계 중심
                `오래된 친구한테 연락이 왔어. "어디야?" 딱 두 글자였는데 눈물이 날 것 같았어. ${subjectCtx} 생각에 혼자 힘들어하고 있었는데, 아직 나를 생각해주는 사람이 있다는 게 이렇게 큰 위로가 될 줄 몰랐거든. ${atmo} 이 감성처럼, 오래된 인연이 주는 온기는 어떤 것도 대신할 수 없어. 바쁘다는 핑계로 잊고 살았는데, 이런 관계가 삶의 가장 큰 재산이라는 걸 다시 느꼈어.`,
                // ④ 내면 독백
                `사실 괜찮지 않아. 주변에는 다 괜찮은 척하지만, 혼자 있을 때는 ${subjectCtx}가 자꾸 마음에 걸려. 모두 앞서가는 것 같고 나만 제자리인 것 같고. 그런데 ${atmo} 이 감성처럼, 어쩌면 각자의 속도가 다를 뿐이라는 걸 머리로는 알고 있어. 마음이 그걸 받아들이는 데 조금 더 시간이 필요한 것뿐이야. 괜찮지 않아도 괜찮은 날이 있다는 걸, 오늘은 그냥 믿어보기로 했어.`,
                // ⑤ 전환과 다짐
                `오늘 SNS를 지웠어. 남들 하이라이트만 보다가 내 일상이 초라해 보이는 게 싫었거든. ${subjectCtx}를 떠올리면서 ${feeling}을 다시 느끼며, 비교는 내가 선택하는 거라는 말이 오늘은 다르게 들렸어. ${atmo} 이 분위기 속에서 이제 남의 삶이 아닌 내 삶을 살기로 했어. 늦지 않았고, 틀리지 않았어. 지금 이 순간부터가 진짜 내 시작이야.`
            ],
            '5060세대': [
                // ① 현재 감정
                `${subjectCtx}를 생각하다가 오랜만에 마음이 조용해졌어요. 30년 넘게 가족과 직장을 위해 달려온 몸이 드디어 잠깐 멈출 수 있는 것 같았어요. ${atmo} 감성처럼, 쉰다는 게 죄가 아니라는 걸 이 나이가 돼서야 몸으로 받아들이게 됐어요. 나를 위한 이 시간이 이기적인 게 아니라 오히려 꼭 필요한 거였다는 걸 알게 됐어요. 수고했어요, 오늘도 그리고 지금까지도.`,
                // ② 과거 회상
                `20년 전 이 나이쯤의 부모님이 문득 떠올랐어요. 그때 부모님이 얼마나 많은 걸 참고 견디셨는지, 이 나이가 돼서야 조금 알 것 같아요. ${themeCtx}에 대해 생각할 때마다 ${feeling}이 담긴 그리움과 감사가 동시에 밀려와요. 그분들이 남겨주신 것들이 지금의 나를 만들었다는 걸, 이제는 완전히 이해해요. 언젠가 다시 만나면 꼭 감사하다고 말하고 싶어요.`,
                // ③ 관계 중심
                `배우자와 함께 ${subjectCtx} 얘기를 나눴어요. 30년을 함께 살아왔는데 아직도 이 사람한테서 배울 게 있다는 게 신기해요. ${atmo} 감성 속에서 배우자가 웃는 걸 보면서, 이 사람이 여전히 좋다는 걸 새삼 확인했어요. 익숙함이 권태가 아니라 편안함이 되는 경지에 이른 것 같아요. ${feeling}처럼, 이것만으로도 충분히 잘 살아온 인생이에요.`,
                // ④ 내면 독백
                `요즘 왜 이렇게 눈물이 자주 나는지 모르겠어요. 뉴스를 봐도, 노래를 들어도, 아이들 사진을 봐도. ${subjectCtx}를 생각하다가도 눈물이 났어요. ${atmo} 이 감성처럼, 인생을 많이 살다 보니 감정의 피부가 얇아지는 것 같아요. 이게 나약한 게 아니라 살아온 것들이 많아서라고 스스로를 다독였어요. 이만큼 깊이 느낄 수 있다는 게 오히려 감사한 일이에요.`,
                // ⑤ 전환과 다짐
                `이 나이에 새로 시작하는 게 두려웠어요. 그런데 ${subjectCtx}를 생각하면서 ${atmo} 이 감성처럼 마음에 따뜻한 용기가 생겼어요. 늦은 시작이 아니라 새로운 시작이에요. 남은 시간을 이미 지난 것들 아쉬워하며 쓰지 않겠어요. 지금 이 순간부터가 진짜 내 인생의 황금기라는 걸, 이제는 알아요.`
            ],
            '시니어세대': [
                // ① 현재 감정
                `${subjectCtx}를 생각하다가 눈물이 차올랐어요. 왜 눈물이 나는지 정확히 설명하기 어려운데, 아마 살아온 세월 전체가 한꺼번에 느껴져서일 거예요. ${atmo} 감성 속에서, 이 눈물이 슬픔이 아니라 감사의 눈물이라는 걸 알아요. ${feeling}처럼 여기까지 살아온 것만으로도 이미 충분히 잘 했어요. 이 마음을 노래로 담을 수 있다면 얼마나 좋을까 싶었어요.`,
                // ② 과거 회상
                `젊은 날의 내가 생각났어요. 얼마나 열심히, 얼마나 바쁘게 살았는지. ${themeCtx}를 떠올리면 그 시절이 ${atmo} 감성처럼 아련하게 느껴져요. 고생도 많았지만 행복도 참 많았어요. 그 모든 날들이 쌓여 지금의 나를 만들었다는 것, 이제는 알아요. ${feeling}처럼, 그 시절의 나에게 고맙고 또 고마워요.`,
                // ③ 관계 중심
                `손주 생각이 났어요. ${subjectCtx}를 생각할 때마다 자연스럽게 그 아이 얼굴이 떠올라요. 아이들의 웃음소리가 세상에서 제일 좋은 노래라는 걸, 이 나이에야 완전히 알겠어요. ${atmo} 감성처럼 ${feeling}이 느껴지는 순간이에요. 그 아이가 건강하게 자라는 것만 봐도 이 생이 충분히 의미있어요.`,
                // ④ 내면 독백
                `평생 가족을 위해, 남을 위해 살았어요. 내 차례는 나중에라고 생각하며 미뤄왔는데, 이렇게 혼자 앉아서 ${subjectCtx}를 생각하다 보니 이게 바로 내 차례구나 싶었어요. ${feeling}이 담긴 ${atmo} 이 감성처럼, 이 여유가 사치가 아니라 당연한 권리였어요. 앞으로는 나를 더 돌봐줘야겠다고 마음먹었어요. 이제는 정말 내 차례예요.`,
                // ⑤ 전환과 다짐
                `살면서 후회되는 게 몇 가지 있어요. 더 자주 사랑한다 말하지 못한 것, 더 많이 안아주지 못한 것. 그런데 ${subjectCtx}를 생각하며 ${atmo} 이 감성을 느끼니, 아직 늦지 않았다는 생각이 들었어요. 오늘 집에 가면 먼저 전화해야겠어요. 고맙다고, 사랑한다고. ${feeling}처럼 진심 어린 그 말을 더 이상 미루지 않을 거예요.`
            ]
        };
