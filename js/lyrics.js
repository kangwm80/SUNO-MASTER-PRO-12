...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 번역 인덱스 캡처/재생 — 영어 가사와 한국어 번역이 같은 문장을 가리키도록
    let trIdxPerTitle = [];   // [[idx,idx,...], [idx,...], ...] 타이틀별 인덱스 저장
    let trCurrentTitle = 0;
    let trCursor = 0;
    let trMode = 'off'; // 'capture' | 'replay' | 'off'
    const state = {
    ...
    function buildKorCtxPools(t, feeling, atmo, p, targetAge) {
        ...
        // 연령층별 말투 적용 헬퍼
        // 같은 내용을 연령층 말투로 변환: 10대·2030 = 반말, 5060 = 해요체, 시니어 = 존댓말
        function e(casual, polite, senior) {
            if (isSenior)  return senior || polite;
            if (isFormal)  return polite;
            return casual;
        }

        // 영어 풀과 인덱스 1:1 대응 — English[i] ↔ Korean[i] 번역 관계
        // Verse short (7) — En: 'In this moment','Looking back now','I remember when','Still holding on','Under the same sky','One step at a time','Right where I am'
        const verseShort = [
            e('이 순간에',          '이 순간에',          '이 순간에'),
            e('돌아보면',           '돌아보면',           '돌아보면'),
            e('그때가 기억나',      '그때가 기억나요',    '그때가 기억납니다'),
            e('아직 붙잡고 있어',   '아직 붙잡고 있어요', '아직 붙잡고 있습니다'),
            e('같은 하늘 아래',     '같은 하늘 아래',     '같은 하늘 아래'),
            e('한 걸음씩',          '한 걸음씩',          '한 걸음씩'),
            e('바로 여기서',        '바로 여기서',        '바로 여기서'),
        ];
        // Verse medium (8)
        const verseMedium = [
            e('내가 처음 시작했던 그 자리에 서 있어',           '제가 처음 시작했던 그 자리에 서 있어요',           '제가 처음 시작했던 그 자리에 서 있습니다'),
            e('모든 기억이 선명하게 쏟아져 나와',               '모든 기억이 선명하게 쏟아져 나와요',               '모든 기억이 선명하게 쏟아져 나옵니다'),
            e('이런 감정이 진짜일 수 있다는 걸 몰랐어',         '이런 감정이 진짜일 수 있다는 걸 몰랐어요',         '이런 감정이 진짜일 수 있다는 걸 몰랐습니다'),
            e('걸어온 길을 보면 내가 얼마나 왔는지 알아',       '걸어온 길을 보면 얼마나 왔는지 알겠어요',           '걸어온 길을 보면 얼마나 왔는지 알겠습니다'),
            e('왜인지 몰라서 헤매던 그 날들이',                 '왜인지 몰라서 헤매던 그 날들이',                   '왜인지 몰라서 헤매던 그 날들이'),
            e('오늘 밤 공기 속에 뭔가 달라졌어',                '오늘 밤 공기 속에 뭔가 달라졌어요',                '오늘 밤 공기 속에 뭔가 달라졌습니다'),
            e('말하지 못한 것들을 찾고 있어',                   '말하지 못한 것들을 찾고 있어요',                   '말하지 못한 것들을 찾고 있습니다'),
            e('시간이 바로 여기서 멈춰버린 것 같아',            '시간이 바로 여기서 멈춰버린 것 같아요',            '시간이 바로 여기서 멈춰버린 것 같습니다'),
        ];
        // Verse long (4)
        const verseLong = [
            e('오늘 밤 이렇게 감정이 밀려올 줄은 몰랐어',
              '오늘 밤 이렇게 감정이 밀려올 줄은 몰랐어요',
              '오늘 밤 이렇게 감정이 밀려올 줄은 몰랐습니다'),
            e('여기까지 데려온 모든 걸음이 결국 의미 있었어',
              '여기까지 데려온 모든 걸음이 결국 의미 있었어요',
              '여기까지 데려온 모든 걸음이 결국 의미 있었습니다'),
            e('이 빛이 다시는 되돌릴 수 없는 순간들을 떠올리게 해',
              '이 빛이 다시는 되돌릴 수 없는 순간들을 떠올리게 해요',
              '이 빛이 다시는 되돌릴 수 없는 순간들을 떠올리게 합니다'),
            e('네가 했던 말 한마디 한마디가 어제처럼 생생해',
              '당신이 했던 말 한마디 한마디가 어제처럼 생생해요',
              '당신이 했던 말 한마디 한마디가 어제처럼 생생합니다'),
        ];

        // Pre-Chorus short (4) — En: 'I cannot let go','This is real now','Here I am','Do not walk away'
        const preShort = [
            e('놓을 수가 없어',     '놓을 수가 없어요',     '놓을 수가 없습니다'),
            e('이건 진짜야',        '이건 진짜예요',         '이건 진짜입니다'),
            e('나 여기 있어',       '저 여기 있어요',        '저 여기 있습니다'),
            e('가지 마',            '가지 마세요',           '가지 마세요'),
        ];
        // Pre-Chorus medium (4)
        const preMedium = [
            e('이 감정을 더 이상 참을 수가 없어',           '이 감정을 더 이상 참을 수가 없어요',           '이 감정을 더 이상 참을 수가 없습니다'),
            e('내 안에 있는 모든 것이 차오르고 있어',       '내 안에 있는 모든 것이 차오르고 있어요',       '내 안에 있는 모든 것이 차오르고 있습니다'),
            e('바로 이 자리에서 버티기로 했어',             '바로 이 자리에서 버티기로 했어요',             '바로 이 자리에서 버티기로 했습니다'),
            e('네가 했던 말 하나하나를 붙잡고 있어',        '당신이 했던 말 하나하나를 붙잡고 있어요',      '당신이 했던 말 하나하나를 붙잡고 있습니다'),
        ];
        // Pre-Chorus long (2)
        const preLong = [
            e('가슴속에서 타오르는 게 느껴져 더 이상 담아둘 수가 없어',
              '가슴속에서 타오르는 게 느껴져요 더 이상 담아둘 수가 없어요',
              '가슴속에서 타오르는 것이 느껴집니다 더 이상 담아둘 수가 없습니다'),
            e('내 모든 부분이 우리 사이의 거리를 넘어 너를 향해 뻗고 있어',
              '제 모든 부분이 우리 사이의 거리를 넘어 당신을 향해 뻗고 있어요',
              '제 모든 부분이 우리 사이의 거리를 넘어 당신을 향해 뻗고 있습니다'),
        ];

        // Chorus short (4) — En: t,'Feel it now','Never let go','Right here with you'
        const chorusShort = [
            t,
            e('지금 느껴봐',            '지금 느껴봐요',          '지금 느껴보세요'),
            e('절대 놓지 마',            '절대 놓지 마세요',        '절대 놓지 마세요'),
            e('바로 여기 너와 함께',    '바로 여기 당신과 함께',   '바로 여기 당신과 함께'),
        ];
        // Chorus medium (6)
        const chorusMedium = [
            e(`${t} 이 밤을 가로질러 너를 부르고 있어`,   `${t} 이 밤을 가로질러 당신을 부르고 있어요`,   `${t} 이 밤을 가로질러 당신을 부르고 있습니다`),
            e('이게 내가 항상 느껴왔던 모든 것이야',      '이게 제가 항상 느껴왔던 모든 것이에요',        '이것이 제가 항상 느껴왔던 모든 것입니다'),
            e(`${t} 이 빛을 통해 빛나고 있어`,            `${t} 이 빛을 통해 빛나고 있어요`,              `${t} 이 빛을 통해 빛나고 있습니다`),
            e('이 순간이 사라지게 두지 않을게',           '이 순간이 사라지게 두지 않을게요',             '이 순간이 사라지게 두지 않겠습니다'),
            e('넌 항상 내가 있어야 할 곳에 있었어',       '당신은 항상 제가 있어야 할 곳에 있었어요',     '당신은 항상 제가 있어야 할 곳에 있었습니다'),
            e(`${t} 이건 진짜고 언제나 그럴 거야`,        `${t} 이건 진짜이고 언제나 그럴 거예요`,         `${t} 이것은 진짜이고 언제나 그럴 것입니다`),
        ];
        // Chorus long (4)
        const chorusLong = [
            e(`${t} 이 감정을 끝까지 꼭 안고 갈게`,       `${t} 이 감정을 끝까지 꼭 안고 갈게요`,         `${t} 이 감정을 끝까지 꼭 안고 가겠습니다`),
            e('이 순간을 마음속에 영원히 담고 갈게',       '이 순간을 마음속에 영원히 담고 갈게요',         '이 순간을 마음속에 영원히 담고 가겠습니다'),
            e(`${t} 내 모든 걸 지금 이 순간에 바칠게`,    `${t} 제 모든 걸 지금 이 순간에 바칠게요`,       `${t} 제 모든 것을 지금 이 순간에 바치겠습니다`),
            e('네 이름을 들을 때마다 이 감정이 내 안에서 솟구쳐',
              '당신의 이름을 들을 때마다 이 감정이 제 안에서 솟구쳐요',
              '당신의 이름을 들을 때마다 이 감정이 제 안에서 솟구칩니다'),
        ];

        // Bridge short (4) — En: 'After all this','I understand now','The answer is clear','It was always you'
        const bridgeShort = [
            e('이 모든 것 끝에',    '이 모든 것 끝에',      '이 모든 것 끝에'),
            e('이제 알겠어',        '이제 알겠어요',         '이제 알겠습니다'),
            e('답이 분명해',        '답이 분명해요',         '답이 분명합니다'),
            e('처음부터 너였어',    '처음부터 당신이었어요', '처음부터 당신이었습니다'),
        ];
        // Bridge medium (4)
        const bridgeMedium = [
            e('구불구불한 길 끝에 마침내 보여',                     '구불구불한 길 끝에 마침내 보여요',                     '구불구불한 길 끝에 마침내 보입니다'),
            e('돌아보니 이제서야 왜인지 알겠어',                     '돌아보니 이제서야 왜인지 알겠어요',                     '돌아보니 이제서야 왜인지 알겠습니다'),
            e('흘린 눈물 한 방울 한 방울이 여기로 이어지고 있었어', '흘린 눈물 한 방울 한 방울이 여기로 이어지고 있었어요', '흘린 눈물 한 방울 한 방울이 여기로 이어지고 있었습니다'),
            e('이 감정이 나를 찾아오기 전까지 난 길을 잃고 있었어', '이 감정이 저를 찾아오기 전까지 저는 길을 잃고 있었어요','이 감정이 저를 찾아오기 전까지 저는 길을 잃고 있었습니다'),
        ];
        // Bridge long (2)
        const bridgeLong = [
            e('수년간의 방황이 마침내 내가 있어야 할 이곳으로 데려왔어',
              '수년간의 방황이 마침내 제가 있어야 할 이곳으로 데려왔어요',
              '수년간의 방황이 마침내 제가 있어야 할 이곳으로 데려왔습니다'),
            e('이제 분명히 보여 나를 이곳으로 이끈 그 길이',
              '이제 분명히 보여요 저를 이곳으로 이끈 그 길이',
              '이제 분명히 보입니다 저를 이곳으로 이끈 그 길이'),
        ];

        // Rap short (2) — En: 'Never giving up','This is my truth'
        const rapShort = [
            e('절대 포기 안 해',    '절대 포기 안 해요',    '절대 포기 안 합니다'),
            e('이게 내 진실이야',   '이게 제 진실이에요',   '이것이 제 진실입니다'),
        ];
        // Rap medium (2)
        const rapMedium = [
            e('내가 있어야 할 곳에 닿을 때까지 절대 멈추지 않을 거야',
              '제가 있어야 할 곳에 닿을 때까지 절대 멈추지 않을 거예요',
              '제가 있어야 할 곳에 닿을 때까지 절대 멈추지 않겠습니다'),
            e('내가 쓴 모든 말은 내 안에서 느끼는 모든 것에서 바로 나왔어',
              '제가 쓴 모든 말은 제 안에서 느끼는 모든 것에서 바로 나왔어요',
              '제가 쓴 모든 말은 제 안에서 느끼는 모든 것에서 바로 나왔습니다'),
        ];
        // Rap long (1)
        const rapLong = [
            e('진심으로 이 노래에 담긴 모든 말 한마디 한마디가 다 진짜야',
              '진심으로 이 노래에 담긴 모든 말 한마디 한마디가 다 진짜예요',
              '진심으로 이 노래에 담긴 모든 말 한마디 한마디가 다 진짜입니다'),
        ];

        // Hook — En: 'Feel it now' / 'Feel it now this ... moment do not let go' / 'Feel this moment never let it fade away'
        const hookShort  = [e('지금 느껴봐', '지금 느껴봐요', '지금 느껴보세요')];
        const hookMedium = [e('지금 이 순간을 느껴봐 놓지 마', '지금 이 순간을 느껴봐요 놓지 마세요', '지금 이 순간을 느껴보세요 놓지 마세요')];
        const hookLong   = [e('이 순간을 느껴봐 네 마음에서 사라지게 두지 마', '이 순간을 느껴봐요 마음에서 사라지게 두지 마세요', '이 순간을 느껴보세요 마음에서 사라지게 두지 마세요')];

        // Outro short (3) — En: t,'Fading out','Always here'
        const outroShort = [
            t,
            e('서서히 사라져',  '서서히 사라져요',  '서서히 사라집니다'),
            e('항상 여기',      '항상 여기',        '항상 여기'),
        ];
        // Outro medium (2)
        const outroMedium = [
            e(`${t} 이 멜로디는 절대 사라지지 않을 거야`,     `${t} 이 멜로디는 절대 사라지지 않을 거예요`,     `${t} 이 멜로디는 절대 사라지지 않겠습니다`),
            e('이런 감정은 항상 돌아오게 되어 있어',           '이런 감정은 항상 돌아오게 되어 있어요',           '이런 감정은 항상 돌아오게 되어 있습니다'),
        ];
        // Outro long (1)
        const outroLong = [
            e(`${t} 이 감정의 모든 순간을 언제나 함께 가져갈게`,
              `${t} 이 감정의 모든 순간을 언제나 함께 가져갈게요`,
              `${t} 이 감정의 모든 순간을 언제나 함께 가져가겠습니다`),
        ];

        return {
            Verse:          { short: verseShort,  medium: verseMedium,  long: verseLong  },
            'Pre-Chorus':   { short: preShort,    medium: preMedium,    long: preLong    },
            Chorus:         { short: chorusShort, medium: chorusMedium, long: chorusLong },
            Bridge:         { short: bridgeShort, medium: bridgeMedium, long: bridgeLong },
            Rap:            { short: rapShort,    medium: rapMedium,    long: rapLong    },
            Hook:           { short: hookShort,   medium: hookMedium,   long: hookLong   },
            'Post-Chorus':  { short: ['오 오 오'], medium: ['오 오 오 예'], long: ['나 나 나 나 나 나'] },
            'Ad-lib':       { short: ['예'], medium: ['같이 예'], long: ['한 번 더 예'] },
            Outro:          { short: outroShort,  medium: outroMedium,  long: outroLong  },
        };
    }
