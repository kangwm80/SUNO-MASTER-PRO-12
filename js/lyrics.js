...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateRecreatedStories(origStories, keywords, themeName) {
        ...
        const sitText = selectedSituation || themeName || '이 순간';
        const place = themeName || '이곳';

        const pool = buildStoryPool(targetGen, place, sitText);
        if (pool.length === 0) return ['스토리를 생성할 수 없습니다. 1단계에서 데이터를 먼저 불러와주세요.'];

        let available = pool.filter(s => !shownStories.has(s));
        ...
        result.forEach(s => shownStories.add(s));
        return result;
    }

    function buildStoryPool(gen, place, sitText) {
        // 파이프라인 데이터 완전 연결
        const d = state.promptData || {};
        const genres = d.genres || [];
        const moods = d.mood || [];
        const themeCtx = place || '이 순간';   // 장소 헤더값 (예: "사랑과 연애", "드라이브")
        const subjectCtx = sitText || themeCtx; // 주제 헤더값 (구체적 상황 문장)

        // 장르 → 감성 매핑
        const gl = genres.map(g => (g || '').toLowerCase()).join(' ');
        let feeling = '따뜻함', genreAtmo = '잔잔하고 감성적인';
        if (gl.includes('발라드') || gl.includes('ballad'))   { feeling = '그리움과 진심';  genreAtmo = '애잔하고 뭉클한'; }
        else if (gl.includes('팝') || gl.includes('pop'))     { feeling = '설렘과 에너지';  genreAtmo = '밝고 생동감 있는'; }
        else if (gl.includes('힙합') || gl.includes('hip'))   { feeling = '도전과 자신감';  genreAtmo = '강렬하고 솔직한'; }
        else if (gl.includes('r&b') || gl.includes('알앤비')) { feeling = '감성과 사랑';    genreAtmo = '부드럽고 감미로운'; }
        else if (gl.includes('트로트') || gl.includes('trot')){ feeling = '흥과 세월';      genreAtmo = '흥겹고 정겨운'; }
        else if (gl.includes('재즈') || gl.includes('jazz'))  { feeling = '여유와 감성';    genreAtmo = '고즈넉하고 감미로운'; }
        else if (gl.includes('록') || gl.includes('rock'))    { feeling = '열정과 자유';    genreAtmo = '뜨겁고 강렬한'; }
        else if (gl.includes('인디') || gl.includes('indie')) { feeling = '감성과 솔직함';  genreAtmo = '감성적이고 자유로운'; }

        // 분위기 → 감성 보완
        const ml = moods.join(' ');
        let moodAtmo = '';
        if (ml.includes('힐링') || ml.includes('편안') || ml.includes('포근')) moodAtmo = '따뜻하고 치유되는';
        else if (ml.includes('그리') || ml.includes('쓸쓸') || ml.includes('센치')) moodAtmo = '아련하고 그리운';
        else if (ml.includes('신나') || ml.includes('흥겨') || ml.includes('파워')) moodAtmo = '신나고 활기찬';
        else if (ml.includes('설레') || ml.includes('사랑')) moodAtmo = '설레고 두근거리는';
        else if (ml.includes('새벽') || ml.includes('몽환')) moodAtmo = '몽환적이고 감성적인';
        else if (ml.includes('위로')) moodAtmo = '따뜻하고 위로가 되는';
        const atmo = moodAtmo || genreAtmo;

        // 세대별 × 5각도 완전 다른 스토리
        // 각도: ①현재감정 ②과거회상 ③관계중심 ④내면독백 ⑤전환다짐
        // 원칙: 후킹 문장으로 시작 → 주제는 스토리 안에 자연스럽게 녹임
        // 관점: 소설가(장면 몰입) + 영화감독(시각적 묘사) + 빌보드 작사가(보편적 공감)
        const pools = {
            '10대': [
                // ① 현재감정 — 후킹: 심장이 먼저 알았어
                `심장이 먼저 알았어. 머리가 설명하기도 전에, 가슴이 쿵 내려앉으면서 '이거다' 싶은 감정이 밀려온 거야. ${subjectCtx}가 갑자기 다른 의미로 다가오면서 평소엔 그냥 지나쳤을 것들이 오늘따라 전부 선명하게 보였어. ${atmo} 이 분위기 속에서 이어폰 꽂고 혼자 걸으면서, 가슴이 꽉 찬 이 기분이 바로 ${feeling}이라는 거구나 싶었어. 말로는 설명 못 하는데 분명히 좋은 감정이고, 이런 순간이 있다는 것 자체가 지금 내가 살아있다는 증거 같아.`,
                // ② 과거회상 — 후킹: 버리지 못한 사진이 하나 있어
                `버리지 못한 사진이 하나 있어. 그때 우리가 얼마나 어렸는지, 지금 보면 웃기면서도 좀 울컥해. ${themeCtx}를 생각할 때마다 그 시절이 떠오르는데, 그때의 나는 지금 이 감정이 얼마나 소중한 건지 전혀 몰랐거든. ${atmo} 그 기억처럼, 지나간 건 다시 못 돌아오지만 그래서 더 빛나는 것 같아. 과거의 내가 있어서 지금의 내가 있다는 게, 오늘은 그냥 고마운 거야.`,
                // ③ 관계중심 — 후킹: 말하려다 삼킨 말이 있어
                `말하려다 삼킨 말이 있어. ${subjectCtx} 얘기를 꺼내려고 열 번은 입을 열었다가 닫았어. 어떻게 말해야 할지 모른다고 했지만, 사실은 내 마음이 진짜인지 확인이 무서웠던 거야. ${atmo} 분위기 속에서 그 사람이 웃는 걸 보면 ${feeling}이 절로 차오르는데, 정작 내 입은 아직 굳어 있어. 말 못 한 마음이 쌓이다 보면 언젠가 노래가 될 것 같아. 그 노래가 완성될 때쯤엔 용기도 같이 생겨있으면 좋겠어.`,
                // ④ 내면독백 — 후킹: 아무도 모르는 게 하나 있어
                `아무도 모르는 게 하나 있어. 웃고 있을 때 사실 울고 싶은 날이 있다는 거. 학교, 학원, 친구 관계, 부모님 기대까지 — 다 버거운데 아무한테도 말 못 해. ${subjectCtx}를 생각할 때만 그 무게가 잠깐 내려가는 것 같아서, 이 시간이 제일 솔직한 순간이야. ${atmo} 이 감성 속에서 억지로 웃지 않아도 되는 이 순간이 제일 편해. 이런 감정을 느끼는 게 약한 게 아니라는 걸, 이제는 조금 알 것 같아.`,
                // ⑤ 전환다짐 — 후킹: 오늘 딱 하나만 바꾸기로 했어
                `오늘 딱 하나만 바꾸기로 했어. 남 눈치 보는 거 그만하기. ${subjectCtx}를 떠올리면서 ${feeling}이 담긴 이 감정이 나만의 것이라는 걸 처음으로 자랑스럽게 느꼈거든. ${atmo} 이 분위기처럼 내 속도대로 살아갈 거야. 아직 어리다고, 몰라서 그렇다고 하는 말들, 이제 흘려보낼게. 지금 내 감정은 진짜고, 그게 제일 중요해.`,
                // ⑥ 새벽감성 — 후킹: 새벽 2시에 혼자 깨어 있는 게 나만은 아닐 거야
                `새벽 2시에 혼자 깨어 있는 게 나만은 아닐 거야. 이런 시간에도 어딘가에서 나랑 똑같이 뒤척이는 애가 있을 것 같아서 조금 위로가 돼. ${subjectCtx}를 생각할 때마다 머릿속이 복잡해지는데, ${atmo} 이 감성이 그걸 조금 녹여주는 것 같아. ${feeling}이 뭔지 지금도 정확히는 모르겠지만, 이 감정이 사라지기 전에 꼭 붙잡고 싶어. 오늘 밤을 버티면 내일은 또 다르게 시작될 거야.`,
                // ⑦ 감각묘사 — 후킹: 이어폰 꽂고 볼륨 높였는데 눈물이 먼저 났어
                `이어폰 꽂고 볼륨 높였는데 눈물이 먼저 났어. 슬픈 노래도 아닌데, 가사 한 줄이 딱 내 얘기 같아서. ${subjectCtx}를 생각하다가 음악 속으로 그냥 잠겨버리고 싶었어. ${atmo} 이 감성처럼, 노래가 말해주는 것들이 내가 하지 못한 말들이라는 걸 알아. ${feeling}이 담긴 이 순간을 언젠가 내가 직접 노래로 만들 수 있다면 얼마나 좋을까 싶어.`,
                // ⑧ 대화체 — 후킹: "요즘 어때?" 물었을 때 솔직하게 대답 못 했어
                `"요즘 어때?" 누가 물었을 때 솔직하게 대답하지 못했어. "응, 그냥 그래" 했는데 사실 전혀 그냥이 아니었거든. ${subjectCtx}가 요즘 계속 마음에 걸리고, ${atmo} 이런 감성이 쌓여서 뭔지 모를 무게감이 있어. 근데 그걸 말로 꺼내는 방법을 아직 잘 모르겠어. ${feeling}처럼, 언젠가 이 마음을 제대로 표현할 수 있는 날이 올 것 같아.`,
                // ⑨ 상상 — 후킹: 만약 내 마음이 노래가 된다면 어떤 멜로디일까
                `만약 내 마음이 노래가 된다면 어떤 멜로디일까, 가끔 상상해. ${subjectCtx}에서 느끼는 이 감정이 악보에 옮겨진다면 빠를까 느릴까. 아마 처음엔 조용히 시작했다가 중간에 터지는 곡일 것 같아. ${atmo} 이 감성처럼, 겉으로는 잔잔한데 속에서는 뭔가 꽉 차 있는 그런 노래. ${feeling}이 담긴 그 노래를 언젠가 누군가가 들으면서 나랑 같은 마음이었다고 느끼면 좋겠어.`,
                // ⑩ 계절/공간 — 후킹: 첫눈 오는 날 창밖 보다가 딱 멈췄어
                `첫눈 오는 날 창밖 보다가 딱 멈췄어. 눈이 오는 동안만큼은 세상이 조용해지는 것 같아서, 그 틈에 ${subjectCtx}가 생각났어. ${atmo} 감성처럼, 눈 내리는 풍경이 이렇게 많은 감정을 불러올 줄 몰랐어. ${feeling}이 담긴 이 순간이 너무 짧게 지나갈까봐 핸드폰 사진 찍는 것도 잊었어. 이런 날들이 더 있었으면 좋겠어.`
            ],
            '2030세대': [
                // ① 현재감정 — 후킹: 퇴근길에 갑자기 발이 멈췄어요
                `퇴근길에 갑자기 발이 멈췄어요. 집에 가야 하는데 아무것도 하기 싫고, 그렇다고 어디 가고 싶지도 않은 그 느낌. ${subjectCtx}가 문득 머릿속을 스쳐가면서, 나 지금 뭘 위해 이렇게 달리고 있는 건지 모르겠다는 생각이 들었어요. ${atmo} 이 감성처럼, 잠깐 멈춰 있는 것도 괜찮다는 걸 오늘은 믿어보고 싶어요. ${feeling}처럼, 충전이 필요하다는 걸 이제는 인정할 수 있어요. 내일의 나를 위해 오늘의 나를 쉬게 해주기로 했어요.`,
                // ② 과거회상 — 후킹: 그때로 돌아갈 수 있다면 뭐가 달라질까
                `그때로 돌아갈 수 있다면 뭐가 달라질까, 가끔 그런 생각을 해요. ${themeCtx}가 지금과는 완전히 다른 의미였던 그 시절 — 아무것도 확실하지 않았는데, 솔직히 지금도 별로 확실한 건 없긴 해요. 그래도 그때의 내가 포기하지 않고 버텨준 덕분에 지금의 내가 있는 거잖아요. ${atmo} 감성처럼, 흔들리면서도 넘어지지 않은 나 자신이 사실 꽤 대단하다는 걸 이제는 알아요. 잘하고 있어요, 진짜로.`,
                // ③ 관계중심 — 후킹: "어디야?" 딱 두 글자에 눈물이 날 뻔했어요
                `"어디야?" 딱 두 글자였는데 눈물이 날 뻔했어요. ${subjectCtx} 생각에 혼자 무너지고 있었는데, 아직 나를 찾아주는 사람이 있다는 게 이렇게 큰 위로가 될 줄 몰랐거든요. ${atmo} 이 감성처럼, 오래된 인연이 주는 온기는 어떤 것도 대신할 수 없어요. 바쁘다는 핑계로 잊고 살았는데, 이런 사람 하나가 삶의 가장 큰 재산이라는 걸 다시 느꼈어요. ${feeling}처럼, 이 온기 꼭 기억해야겠어요.`,
                // ④ 내면독백 — 후킹: 사실 저 요즘 괜찮지 않아요
                `사실 저 요즘 괜찮지 않아요. 다들 앞서가는 것 같고 나만 제자리인 것 같고 — ${subjectCtx}가 자꾸 마음에 걸려서 혼자 있을 때마다 무거워요. 그런데 ${atmo} 이 감성처럼, 어쩌면 각자의 속도가 다를 뿐이라는 걸 머리로는 알고 있어요. 마음이 그걸 받아들이는 데 조금 더 시간이 필요한 것뿐이에요. 괜찮지 않아도 괜찮은 날이 있다는 걸, 오늘은 그냥 믿어보기로 했어요.`,
                // ⑤ 전환다짐 — 후킹: 오늘 SNS 앱을 지웠어요
                `오늘 SNS 앱을 지웠어요. 남들 하이라이트만 보다가 내 일상이 자꾸 초라해 보이는 게 싫었거든요. ${subjectCtx}를 떠올리면서 ${feeling}을 다시 느끼니, 비교는 내가 선택하는 거라는 말이 오늘은 진짜로 들렸어요. ${atmo} 이 분위기 속에서 이제 남의 삶이 아닌 내 삶을 살기로 했어요. 늦지 않았고, 틀리지 않았어요. 지금 이 순간부터가 진짜 내 시작이에요.`,
                // ⑥ 새벽감성 — 후킹: 새벽 3시에 혼자 창문 바라보다가
                `새벽 3시에 혼자 창문 바라보다가, 이 도시가 이렇게 조용한 시간이 있었나 싶었어요. ${subjectCtx}를 생각하면서 요즘 내가 너무 많은 걸 혼자 짊어지고 있는 건 아닌가 싶기도 하고. ${atmo} 이 감성 속에서 잠 못 드는 밤이 꼭 나쁜 건 아닐 수도 있어요. 이 조용함 안에서 진짜 내 목소리가 들리는 것 같아서. ${feeling}처럼, 내일은 조금 더 나한테 솔직하게 살아야겠어요.`,
                // ⑦ 감각묘사 — 후킹: 커피 한 모금에 오늘 하루가 통째로 녹아드는 것 같았어요
                `커피 한 모금에 오늘 하루가 통째로 녹아드는 것 같았어요. 따뜻한 게 이렇게 위로가 될 줄은 몰랐어요. ${subjectCtx}를 생각하며 ${atmo} 이 감성 속에 잠겨있으니, 바쁘게 달리던 속도가 잠깐 느려졌어요. ${feeling}처럼 작은 것들이 실은 제일 큰 위로라는 걸, 이런 순간에 깨달아요. 내일도 이런 여백이 있으면 좋겠어요.`,
                // ⑧ 대화체 — 후킹: "잘 지내?" 물어봤을 때 "응" 하고 말았어요
                `"잘 지내?"라는 말에 그냥 "응" 하고 말았어요. 진짜 대답은 훨씬 길었는데. ${subjectCtx}가 요즘 자꾸 마음에 걸리는데 선뜻 꺼내기가 어렵더라고요. ${atmo} 감성처럼, 가까운 사람한테 솔직해지는 게 가끔 제일 어렵잖아요. ${feeling}처럼 말 못 한 감정들이 쌓이다 보면 언젠가는 터질 것 같아서, 오늘은 조금 더 솔직하게 연습해보려고요.`,
                // ⑨ 상상 — 후킹: 5년 후의 나에게 편지를 쓴다면 뭐라고 할까요
                `5년 후의 나에게 편지를 쓴다면 뭐라고 할까요. '그때 네가 힘들어했던 거, 다 괜찮아졌어'라고 쓸 수 있으면 좋겠어요. ${subjectCtx}를 지금 이렇게 느끼는 것도, ${atmo} 감성처럼 나중에 보면 그냥 지나가는 한 장면이겠죠. ${feeling}이 담긴 이 시간이 결국 나를 만들어가는 과정이라고, 미래의 나는 웃으면서 말해줄 수 있을 것 같아요.`,
                // ⑩ 계절/공간 — 후킹: 벚꽃 떨어지는 걸 보면서 올해도 빠르다고 생각했어요
                `벚꽃 떨어지는 걸 보면서 올해도 빠르다고 생각했어요. ${subjectCtx}를 생각할 겨를도 없이 봄이 가버리는 것 같아서, 잠깐 걸음을 멈추고 올려다봤어요. ${atmo} 이 감성처럼, 빠르게 떨어지는 꽃잎이 슬프지 않은 건 다음에 또 피어난다는 걸 알기 때문이에요. ${feeling}처럼, 나도 그렇게 다시 피어날 수 있다는 걸 봄이 올 때마다 믿어요.`
            ],
            '5060세대': [
                // ① 현재감정 — 후킹: 손이 떨렸어요, 나를 위해 뭔가를 결정하는 순간에
                `손이 떨렸어요. 처음으로 나 자신만을 위해 뭔가를 결정하는 순간에. 30년 넘게 가족과 직장을 위해 달려온 사람이 잠깐 멈추는 게 이렇게 낯설 줄 몰랐어요. ${subjectCtx}를 생각하면서 ${atmo} 이 감성 속에서, 쉰다는 게 죄가 아니라는 걸 몸으로 받아들였어요. 나를 위한 이 시간이 이기적인 게 아니라 꼭 필요한 거였다는 걸 알게 됐어요. 수고했어요, 오늘도 그리고 지금까지도.`,
                // ② 과거회상 — 후킹: 그 나이쯤 됐을 때 부모님 얼굴이 떠올랐어요
                `이 나이쯤 됐을 때 우리 부모님 얼굴이 떠올랐어요. 그때 그분들이 얼마나 많은 걸 참고 견디셨는지, 이제야 조금 알 것 같아요. ${themeCtx}를 생각할 때마다 ${feeling}이 담긴 그리움과 감사가 동시에 밀려와요. 그분들이 남겨주신 것들이 지금의 나를 만들었다는 걸 이제는 완전히 이해해요. 언젠가 다시 만나면 꼭 감사하다고 말하고 싶어요.`,
                // ③ 관계중심 — 후킹: 30년을 살아도 아직 이 사람이 좋아요
                `30년을 살아도 아직 이 사람이 좋아요. ${subjectCtx} 얘기를 나누다가 배우자가 웃는 걸 보면서 새삼 그 마음을 확인했어요. 익숙함이 권태가 아니라 편안함이 되는 경지, 이게 바로 ${atmo} 이 감성이에요. ${feeling}처럼, 이 사람과 함께한 시간이 내 인생에서 가장 잘한 일이에요. 이것만으로도 충분히 잘 살아온 인생이에요.`,
                // ④ 내면독백 — 후킹: 요즘 이유 없이 눈물이 나요
                `요즘 이유 없이 눈물이 나요. 뉴스를 봐도, 노래를 들어도, 아이들 사진을 봐도. ${subjectCtx}를 생각하다가도 눈물이 났어요. ${atmo} 이 감성처럼, 인생을 많이 살다 보니 감정의 피부가 얇아지는 것 같아요. 이게 나약한 게 아니라 살아온 것들이 많아서라는 걸, 오늘은 스스로에게 말해줬어요. 이만큼 깊이 느낄 수 있다는 게 오히려 감사한 일이에요.`,
                // ⑤ 전환다짐 — 후킹: 이 나이에 처음이라는 말을 쓸 줄 몰랐어요
                `이 나이에 처음이라는 말을 쓸 줄 몰랐어요. 그런데 ${subjectCtx}를 생각하면서 ${atmo} 이 감성처럼 마음에 따뜻한 용기가 생겼어요. 늦은 시작이 아니라 새로운 시작이에요. 남은 시간을 이미 지난 것들 아쉬워하며 쓰지 않겠어요. 지금 이 순간부터가 진짜 내 인생의 황금기예요.`,
                // ⑥ 새벽감성 — 후킹: 새벽에 혼자 일어나 마시는 커피 한 잔이 제일 솔직한 시간이에요
                `새벽에 혼자 일어나 마시는 커피 한 잔이 하루 중 제일 솔직한 시간이에요. 아무도 없는 그 조용함 속에서 ${subjectCtx}를 생각하면, ${atmo} 이 감성처럼 많은 감정이 자연스럽게 차올라요. 이 나이쯤 되면 이런 새벽 시간이 두렵지 않아요. 오히려 나를 제일 잘 만나는 시간이에요. ${feeling}처럼, 이 고요함 안에서 내가 걸어온 길이 참 괜찮았다는 걸 확인해요.`,
                // ⑦ 감각묘사 — 후킹: 구수한 냄새에 갑자기 부모님이 떠올랐어요
                `구수한 냄새에 갑자기 부모님이 떠올랐어요. ${themeCtx}를 생각할 때마다 그분들이 생각나는데, ${atmo} 감성처럼 그 기억이 슬프지 않고 따뜻해요. 그때는 몰랐어요, 그게 얼마나 소중한 시간인지. ${feeling}이 담긴 그 시절을 이제는 감사하게 돌아볼 수 있어요. 나도 내 아이들에게 그런 따뜻한 기억이 되고 싶어요.`,
                // ⑧ 대화체 — 후킹: "다음에 보자"는 말이 이렇게 무거울 줄 몰랐어요
                `"다음에 보자"는 말이 이렇게 무거울 줄 몰랐어요. 이 나이가 되고 나서야 '다음'이 보장된 게 아니라는 걸 알거든요. ${subjectCtx}를 생각하며 ${atmo} 이 감성처럼 지금 이 순간이 얼마나 소중한지 새삼 느껴요. ${feeling}처럼, 오늘 만나는 사람에게 더 진심으로 대하고 싶어요. 나중에 후회하지 않도록, 지금을 아끼면서 살아야겠어요.`,
                // ⑨ 상상 — 후킹: 내 인생이 영화라면 지금은 어떤 장면일까요
                `내 인생이 영화라면 지금은 어떤 장면일까요. 클라이막스는 이미 지났고, 이제는 아름다운 마무리를 향해 가는 장면인 것 같아요. ${subjectCtx}를 생각하면서 ${atmo} 이 감성처럼, 이 영화에서 제일 아름다운 장면이 아직 찍히지 않았다는 느낌이 들어요. ${feeling}처럼, 남은 시간을 더 빛나게 채워가고 싶어요. 아직 다 쓰지 못한 이야기가 있어요.`,
                // ⑩ 계절/공간 — 후킹: 단풍이 드는 걸 보면서 세월이 참 빠르다고 생각했어요
                `단풍이 드는 걸 보면서 세월이 참 빠르다는 걸 다시 느꼈어요. ${themeCtx}를 생각할 때마다 이 계절처럼 색이 달라지는 내 인생이 떠올라요. ${atmo} 감성처럼, 변해가는 것들이 꼭 슬픈 건 아니에요. 물드는 것도 얼마나 아름다운 과정인지, 단풍이 가르쳐줘요. ${feeling}처럼, 나도 지금 이 계절을 충분히 아름답게 살고 있어요.`
            ],
            '시니어세대': [
                // ① 현재감정 — 후킹: 눈물이 났어요, 왜 나는지 설명할 수가 없는데
                `눈물이 났어요. 왜 나는지 설명할 수가 없는데, 아마 살아온 세월 전체가 한꺼번에 느껴져서일 거예요. ${subjectCtx}를 생각하는데 그냥 눈물이 차올랐어요. ${atmo} 이 감성 속에서, 이 눈물이 슬픔이 아니라 감사의 눈물이라는 걸 알아요. ${feeling}처럼, 여기까지 살아온 것만으로도 이미 충분히 잘한 거예요. 이 마음을 노래로 담을 수 있다면 얼마나 좋을까 싶었어요.`,
                // ② 과거회상 — 후킹: 그때 나는 정말 열심히 살았어요
                `그때 나는 정말 열심히 살았어요. 지금 생각하면 어떻게 그 시간을 버텼는지 신기할 정도로. ${themeCtx}를 떠올리면 그 시절이 ${atmo} 감성처럼 아련하게 느껴져요. 고생도 많았지만 행복도 참 많았어요. 그 모든 날들이 쌓여 지금의 나를 만들었다는 것, 이제는 알아요. ${feeling}처럼, 그 시절의 나에게 고맙고 또 고마워요.`,
                // ③ 관계중심 — 후킹: 손주 얼굴이 떠올랐어요
                `손주 얼굴이 떠올랐어요. ${subjectCtx}를 생각할 때마다 자연스럽게 그 아이 웃음소리가 들리는 것 같아요. 아이들의 웃음이 세상에서 제일 좋은 노래라는 걸, 이 나이에야 완전히 알겠어요. ${atmo} 감성처럼 ${feeling}이 느껴지는 이 순간, 그 아이가 건강하게 자라는 것만 봐도 이 생이 충분히 의미있어요. 다음엔 꼭 함께 오고 싶어요.`,
                // ④ 내면독백 — 후킹: 평생 남을 위해 살았는데, 오늘은 내 차례예요
                `평생 남을 위해 살았어요. 내 차례는 나중에라고 생각하며 미뤄왔는데, 오늘 혼자 앉아 ${subjectCtx}를 생각하다 보니 — 이게 바로 내 차례구나 싶었어요. ${feeling}이 담긴 ${atmo} 이 감성처럼, 이 여유가 사치가 아니라 당연한 권리였어요. 앞으로는 나를 더 돌봐줘야겠어요. 이제는 정말 내 차례예요.`,
                // ⑤ 전환다짐 — 후킹: 후회되는 게 있어요, 더 자주 사랑한다 말하지 못한 것
                `후회되는 게 있어요. 더 자주 사랑한다 말하지 못한 것, 더 많이 안아주지 못한 것. 그런데 ${subjectCtx}를 생각하며 ${atmo} 이 감성을 느끼니, 아직 늦지 않았다는 생각이 들었어요. 오늘 집에 가면 먼저 전화할 거예요. 고맙다고, 사랑한다고. ${feeling}처럼 진심 어린 그 말을 더 이상 미루지 않을 거예요.`,
                // ⑥ 새벽감성 — 후킹: 새벽 일찍 눈이 떠지는 게 요즘 버릇이 됐어요
                `새벽 일찍 눈이 떠지는 게 요즘 버릇이 됐어요. 몸이 쉬어도 마음은 깨어있으려 하는 것 같아서. ${subjectCtx}를 생각하면서 ${atmo} 이 감성 속에서 혼자 앉아 있는 새벽이 외롭지 않아요. 지금껏 살아온 게 다 이 마음 안에 있으니까요. ${feeling}처럼, 이 조용한 새벽이 제일 풍요로운 시간이에요. 오늘 하루도 감사하게 살아야겠어요.`,
                // ⑦ 감각묘사 — 후킹: 텃밭에서 막 딴 것들이 이렇게 달 줄이야
                `텃밭에서 막 딴 것들이 이렇게 달 줄이야. ${subjectCtx}를 생각할 때마다 이 작은 기쁨들이 겹쳐서 떠올라요. ${atmo} 감성처럼, 손으로 직접 키운 것의 달콤함은 어디서도 살 수 없어요. ${feeling}이 담긴 이 순간들이 쌓여서 노년이 풍요롭다는 걸, 이제는 알아요. 이런 작은 것들이 제일 큰 행복이에요.`,
                // ⑧ 대화체 — 후킹: 오랜 친구 목소리를 들으니 젊은 날이 훅 돌아왔어요
                `오랜 친구 목소리를 들으니 젊은 날이 훅 돌아왔어요. ${subjectCtx}를 얘기하다가 우리가 얼마나 오래됐는지 실감했어요. ${atmo} 감성처럼, 이렇게 오래 이어진 인연이 삶에서 제일 귀한 재산이에요. ${feeling}이 담긴 그 시절을 함께한 사람이 있다는 게 얼마나 든든한지 몰라요. 살아있는 동안 더 자주 목소리를 들어야겠어요.`,
                // ⑨ 상상 — 후킹: 하늘에서 내 인생을 내려다본다면 어떻게 보일까요
                `하늘에서 내 인생을 내려다본다면 어떻게 보일까요. 꼬불꼬불하고 험했던 길도 있었지만, 결국 여기까지 왔네요. ${subjectCtx}를 생각하면서 ${atmo} 이 감성처럼, 그 굽이굽이가 다 의미 있었다는 걸 이제는 알아요. ${feeling}이 담긴 내 이야기가 누군가에게 힘이 된다면, 그것만으로도 이 삶은 충분히 빛났어요.`,
                // ⑩ 계절/공간 — 후킹: 봄이 오는 걸 보면서 나도 다시 피어날 수 있겠다 싶었어요
                `봄이 오는 걸 보면서 나도 다시 피어날 수 있겠다 싶었어요. ${subjectCtx}를 생각하며 ${atmo} 이 감성처럼, 계절이 돌아오듯 삶에도 새 봄이 오는 거라고 믿어요. ${feeling}이 담긴 이 설렘이 이 나이에도 느껴진다는 게 참 감사해요. 나이는 숫자일 뿐이에요. 마음이 피어있으면 언제나 봄이에요.`
            ]
        };
        return pools[gen] || pools['2030세대'] || [];
    }
    ...
    function buildContextPools(title, gen, lang) {
        ...
        if (lang === 'korean') return buildKorCtxPools(t, feeling, atmo, p, targetAge);
        return buildEngCtxPools(t, feelingEng, atmoEng, p);
    }

    function buildKorCtxPools(t, feeling, atmo, p, targetAge) {
        // 연령층별 일상 언어 사전 — 각 세대가 실제로 쓰는 말투로 차별화
        const isFormal  = targetAge === '5060세대' || targetAge === '시니어세대';
        const isYoung   = targetAge === '10대';
        const isMid     = targetAge === '2030세대';
        const isSenior  = targetAge === '시니어세대';
        ...
        // ── 연령층별 Verse 문장 풀 ──
        let verseShort, verseMedium, verseLong;

        if (isYoung) {
            // 10대: 학교/SNS/첫사랑, 짧고 강렬, 줄임말·감탄사 자연스럽게
            verseShort  = ['완전 설레', '진짜 좋아', '헐 어떡해', '갓 감성', `${p}에서`, '단톡방 봐도', '수업 중에도'];
            verseMedium = [
                '완전 설레 처음 봤을 때부터', '진짜로 좋아하는 거 들키면 어떡해',
                '단톡방에서만 용감한 내 마음', '수업 중에도 네 생각만 나',
                '교복 입고 달려갔던 그 길이', '헐 이게 뭐지 심장이 이상해',
                '카톡 읽씹해도 또 보내는 나', '진짜 갓 같아 너는 완전히'
            ];
            verseLong   = [
                '완전 설레 진짜로 좋아하는 거 티 나면 어떡해',
                '단톡방에서만 용감했던 내가 오늘은 직접 말할 거야',
                '수업 끝나고 교문 앞에서 기다렸던 거 알아?',
                '인스타 올리고 싶었는데 용기가 없었던 그 날들'
            ];
        } else if (isMid) {
            // 2030세대: 퇴근/번아웃/혼밥/자취, 공감되는 현실 언어
            verseShort  = ['또 야근이야', '혼밥 중이야', '지쳤어 오늘', `${p}에서`, '현타 왔어', '그냥 쉬고파', '괜찮은 척'];
            verseMedium = [
                '오늘도 야근하고 집에 가는 길에', '혼자 밥 먹으면서 폰만 보는 밤에',
                '번아웃인 건지 그냥 지친 건지 몰라', '월급날인데 통장은 여전히 비어있어',
                '괜찮은 척했는데 사실 안 괜찮았어', '커피 한 잔으로 하루를 버텨내는 중',
                '퇴근하고 집 오면 아무것도 하기 싫어', '소확행이라도 없으면 살 것 같아'
            ];
            verseLong   = [
                '번아웃인 건지 지친 건지 모르겠는데 그냥 쉬고 싶어',
                '오늘도 야근하고 지하철에서 혼자 서 있는데 갑자기 네 생각이 나',
                '월급날인데 통장 잔액 보니까 웃음밖에 안 나오는 거 공감돼?',
                '괜찮다고 했는데 사실은 누군가 그냥 물어봐줬으면 했어'
            ];
        } else if (isFormal && !isSenior) {
            // 5060세대: 퇴근길/가족/수고, 따뜻하고 진심 어린 어른 말투
            verseShort  = ['수고했어요', '오늘도 참 많이', '가족 생각에', `${p}에서`, '힘들었지만', '그래도 버텼어', '여기까지 왔어'];
            verseMedium = [
                '오늘 하루도 참 고생 많이 하셨어요', '퇴근길에 불현듯 옛날 생각이 나서',
                '아이들 키우느라 참 많이 수고하셨죠', '그래도 여기까지 왔으니 잘 한 거예요',
                '힘들었지만 포기하지 않아서 다행이에요', '가족들 얼굴 보면 피곤함이 사라져요',
                '당신이 있어서 우리 가족이 행복해요', '수고했어요 오늘 하루도 정말로'
            ];
            verseLong   = [
                '오늘 하루도 참 고생 많으셨어요 정말 수고하셨습니다',
                '퇴근길 지하철에서 불현듯 옛날 생각이 나서 웃음이 났어요',
                '아이들 다 크고 나니까 그때 그 시절이 얼마나 소중했는지 알겠어요',
                '힘들었지만 포기하지 않고 여기까지 온 당신이 정말 대단해요'
            ];
        } else if (isSenior) {
            // 시니어: 건강/가족/감사/추억, 정중하고 따뜻한 말투
            verseShort  = ['감사해요', '건강하세요', '행복한 날', `${p}에서`, '세월이 참', '가족이 있어', '고맙습니다'];
            verseMedium = [
                '세월이 참 빠르게도 흘러갔네요', '가족들이 있어서 정말 감사해요',
                '건강하게 함께해서 행복합니다', '오늘 하루도 감사한 마음이에요',
                '당신이 곁에 있어서 든든해요', '손자 손녀 웃는 얼굴이 제일이에요',
                '이 나이에 이렇게 행복해도 되나요', '고마운 사람들이 참 많이 있어요'
            ];
            verseLong   = [
                '세월이 흘러도 변하지 않는 건 가족에 대한 사랑이에요',
                '건강하게 오래오래 함께할 수 있어서 정말 감사합니다',
                '손자 손녀 웃는 얼굴 보면 이 세상 부럽지 않아요',
                '돌아보면 힘든 날도 있었지만 지금 이 순간이 제일 행복해요'
            ];
        } else {
            // 기본(공통): 자연스러운 일상 감성
            verseShort  = ['그 날처럼', '혼자 걷다', '눈을 감아', '기억 속에', '아직 여기', `${p}에서`, '오늘도 또'];
            verseMedium = [
                `${atmo} 이 길을 걷다 보면`, `${p}에서 멈춰 서는 순간`,
                '오늘도 같은 길인데 달라', '말하지 못한 것들이 쌓여',
                '아직도 그 날이 눈에 보여', '시간이 멈춰버린 것 같아',
                '익숙한 향기에 네가 떠올라', '창밖에 비가 내리던 그 날'
            ];
            verseLong   = [
        ...
        // ── Chorus는 제목 + 연령층별 핵심 감정 ──
        let chorusMedium, chorusLong;
        if (isYoung) {
            chorusMedium = [`${t} 진짜 너만 보여`, '이 순간 완전 설레', `${t} 갓 같아 너는`, '심장이 폭발할 것 같아', '미쳤어 이 감정이'];
            chorusLong   = [`${t} 진짜로 좋아해 티 나도 상관없어`, `완전 설레 이 감정 ${t} 네 이름처럼`];
        } else if (isMid) {
            chorusMedium = [`${t} 그냥 네가 보고 싶어`, '괜찮다 거짓말하지 마', `${t} 충분해 지금 이대로`, '천천히 가도 괜찮아', '혼자가 아니야'];
            chorusLong   = [`${t} 힘들어도 괜찮아 그냥 네 옆에 있고 싶어`, `번아웃 와도 ${t} 이 노래 들으면 조금 낫잖아`];
        } else if (isSenior) {
            chorusMedium = [`${t} 감사해요 정말로`, '건강하게 함께해요', `${t} 행복해요 당신과`, '고마워요 오늘도', '사랑합니다 언제나'];
            chorusLong   = [`${t} 당신이 있어서 이 하루가 행복합니다`, `건강하게 오래오래 함께해요 ${t}`];
        } else {
            chorusMedium = [`${t} 너에게 닿을 때까지`, '이 순간을 영원히 기억할게', `${t} 내 마음을 담아서`, `${feeling}이 담긴 이 노래가 들려`];
            chorusLong   = [`${t} 이 노래가 너에게 전해질 때 알아줘`, `${atmo} 이 빛 속에서 ${feeling}이 가득 차오르는 것 같아`];
        }

        // ── 연령층별 Pre-Chorus ──
        let preShort, preMedium, preLong;
        if (isYoung) {
            preShort  = ['말 못 하겠어', '심장 터질 것 같아', '어떻게 하지', '이대로 들킬 것 같아'];
            preMedium = ['이 마음 들키기 전에 도망쳐야 하나', '심장이 너무 빨리 뛰어서 무서워', '어떡해 눈 마주쳤어 완전 떨려', '말해버릴까 아니 아직은 아냐'];
            preLong   = ['이 마음 말하면 어떻게 될지 몰라서 너무 무서워', '심장이 터질 것 같은데 표정은 괜찮은 척해야 해'];
        } else if (isMid) {
            preShort  = ['그냥 솔직하게', '이대로는 아냐', '뭔가 달라져야 해', '지금이 기회야'];
            preMedium = ['이대로 지내다가 나중에 후회할 것 같아', '솔직하게 말하지 못한 게 가장 힘들어', '뭔가 달라지고 싶어서 여기까지 버텼어', '이쯤에서 한번 제대로 쉬고 싶어'];
            preLong   = ['이대로 아무것도 안 하면 나중에 진짜 후회할 것 같아서', '솔직하게 말 못 한 게 쌓이다 보니 여기까지 왔어'];
        } else if (isSenior) {
            preShort  = ['감사한 마음으로', '오래오래 함께', '건강하길 바라요', '한 가지만 말해요'];
            preMedium = ['이렇게 건강하게 함께할 수 있어서 감사해요', '살면서 후회 없이 살았다고 말할 수 있어요', '가족이 곁에 있으면 아무것도 부럽지 않아요', '이 나이가 되니까 진짜 소중한 것만 보여요'];
            preLong   = ['이렇게 행복한 날들이 계속되길 진심으로 바랍니다', '살아온 날보다 앞으로의 날이 더 소중하다는 걸 알아요'];
        } else if (isFormal && !isSenior) {
            preShort  = ['그래도 괜찮아요', '이제 알겠어요', '포기하지 않을게요', '함께라서 다행이에요'];
            preMedium = ['그래도 여기까지 왔으니 잘 하고 있는 거예요', '힘들 때 옆에 있어준 사람이 생각나요', '포기하고 싶었지만 결국 버텨낸 내가 대단해요', '함께해서 이 시간이 더 소중하게 느껴져요'];
            preLong   = ['힘든 시간을 버텨온 당신이 얼마나 대단한지 이제야 알겠어요', '그 어렵던 시절도 결국 지나갔으니 지금 이 순간이 더 빛나요'];
        } else {
            preShort  = ['이대로 멈추지 마', '조금만 더 가면 돼', '포기하지 말아줘', '지금이 그 순간이야'];
            preMedium = ['이대로 멈추면 안 될 것 같아 조금만 더 가자', '포기하고 싶은 순간에도 걸어온 길이 있잖아', '지금 이 느낌이 진짜라는 걸 믿고 싶어', '조금만 더 가면 거기에 네가 있을 것 같아'];
            preLong   = ['조금만 더 가면 거기에 네가 있을 것 같아서 멈출 수가 없어', '포기하고 싶어도 이 감정이 사라지지 않아서 계속 걷고 있어'];
        }

        // ── 연령층별 Bridge ──
        let bridgeShort, bridgeMedium, bridgeLong;
        if (isYoung) {
            bridgeShort  = ['사실은 말이야', '이제 말할게', '숨길 수 없어', '들켜도 괜찮아'];
            bridgeMedium = ['사실 처음부터 좋아했어 용기가 없었을 뿐', '이제 더 이상 숨길 수가 없어 진짜로', '들켜도 괜찮아 이 감정이 진짜니까', '후회하기 전에 말해야 할 것 같아'];
            bridgeLong   = ['처음부터 좋아했는데 용기가 없어서 말을 못 했어 이제는 말할게', '들켜도 괜찮아 어차피 이 마음 숨길 수가 없으니까 그냥 말해버릴게'];
        } else if (isMid) {
            bridgeShort  = ['그래도 괜찮아', '다 잘될 거야', '버텨온 내가 대단해', '조금만 쉬어가자'];
            bridgeMedium = ['지쳐도 괜찮아 여기까지 온 것만으로 충분해', '버텨온 내가 사실은 정말 대단한 거였어', '다 잘 될 거야 지금은 힘들어도', '한 번쯤 쉬어가도 지는 게 아니야'];
            bridgeLong   = ['지쳐도 괜찮아 여기까지 온 것만으로 이미 충분히 잘하고 있는 거야', '버텨온 내 자신한테 수고했다고 말해줄 수 있어야 한다는 거 이제 알아'];
        } else if (isSenior) {
            bridgeShort  = ['돌아보면 참', '후회 없어요', '감사할 뿐이에요', '행복했어요'];
            bridgeMedium = ['돌아보면 힘든 날도 있었지만 다 감사한 일이에요', '후회 없이 살았다고 자신 있게 말할 수 있어서 좋아요', '행복했던 날들이 더 많았으니 그걸로 충분해요', '가족들과 함께한 모든 순간이 다 소중한 추억이에요'];
            bridgeLong   = ['돌아보면 힘든 날도 있었지만 그 시간이 있었기에 지금 이 행복이 더 소중해요', '후회 없이 살아온 것 같아요 가족들과 함께한 시간이 있었으니까요'];
        } else if (isFormal && !isSenior) {
            bridgeShort  = ['이제 알았어요', '그때를 보면', '고마웠어요', '잘 해왔어요'];
            bridgeMedium = ['이제야 알았어요 그 모든 시간이 여기로 이어진다는 걸', '그때 힘들었던 게 지금의 나를 만들었다는 게 신기해요', '고마웠어요 힘든 때도 곁에 있어줬던 그 사람이', '잘 해왔어요 돌아보면 정말 열심히 살았어요'];
            bridgeLong   = ['이제야 알겠어요 그 힘들었던 시간들이 결국 지금의 나를 만들었다는 걸', '고마워요 그때 포기하지 말라고 옆에서 말해줬던 당신이 있어서 여기까지 왔어요'];
        } else {
            bridgeShort  = ['이제야 알겠어', '그게 너였어', '처음부터 너였어', '결국엔 너야'];
            bridgeMedium = ['이제야 알겠어 그 모든 게 너로 통하고 있었다는 걸', '돌아보면 항상 그 자리에 네가 있었어', '처음부터 너였을지도 몰라 이 감정이', '결국엔 너야 어디를 돌아봐도'];
            bridgeLong   = ['이제야 알겠어 처음부터 네가 이 노래의 주인공이었다는 걸 왜 몰랐을까', '돌아보면 항상 그 자리에 네가 있었어 내가 보지 못했던 것뿐이었어'];
        }

        // ── 연령층별 Rap ──
        let rapShort, rapMedium, rapLong;
        if (isYoung) {
            rapShort  = ['솔직히 말해', '들어봐 잠깐'];
            rapMedium = ['솔직히 말해 처음부터 좋았어 티 안 내려고 했는데', '교복 입고 달려간 그 교문 앞에서 기다리던 그 시간이'];
            rapLong   = ['솔직히 말할게 처음부터 좋았어 단톡방에서만 용감했던 내가 오늘은 직접 말하는 거야'];
        } else if (isMid) {
            rapShort  = ['현실적으로 봐', '들어봐 솔직히'];
            rapMedium = ['야근하고 집에 오면 아무것도 하기 싫은데 이 노래만 들으면 조금 나아', '번아웃이든 뭐든 간에 일단 오늘 하루는 버텼으니까 그걸로 됐어'];
            rapLong   = ['월급날 통장 보면 웃음만 나오는 거 공감하지 그래도 오늘 하루 버텼으니까 그거면 충분해'];
        } else if (isSenior) {
            rapShort  = ['살아보니까', '이제 알아요'];
            rapMedium = ['살아보니까 진짜 소중한 게 뭔지 이제는 알겠어요', '건강하고 가족 곁에 있는 것 그게 제일 큰 행복이에요'];
            rapLong   = ['살면서 배운 것들이 있어요 건강이 최고고 가족이 전부라는 것 이제는 진심으로 알아요'];
        } else if (isFormal && !isSenior) {
            rapShort  = ['그래도 했어요', '버텼으니까요'];
            rapMedium = ['힘들었지만 포기 안 했어요 여기까지 왔으니 잘 한 거예요', '버텨온 시간들이 쌓여서 지금의 내가 됐다는 걸 알아요'];
            rapLong   = ['힘들었지만 포기하지 않았어요 그리고 여기까지 왔어요 그것만으로도 충분히 대단한 거예요'];
        } else {
            rapShort  = ['그냥 솔직히', '들어봐 잠깐'];
            rapMedium = ['그냥 솔직하게 말할게 이 감정이 뭔지 나도 몰랐어 근데 이제는 알아', '말하지 못한 것들이 노래가 됐어 이 멜로디에 다 담겨있어'];
            rapLong   = ['그냥 솔직하게 말할게 이 노래 전부가 너한테 하고 싶었던 말들이야 이제 들어봐'];
        }

        // ── 연령층별 Hook ──
        let hookShort, hookMedium, hookLong;
        if (isYoung) {
            hookShort  = [`${t} 완전 좋아`];
            hookMedium = [`${t} 완전 좋아 이 순간 영원했으면`];
            hookLong   = [`${t} 완전 좋아 이 순간 영원히 멈춰버렸으면 좋겠어`];
        } else if (isMid) {
            hookShort  = [`${t} 괜찮아`];
            hookMedium = [`${t} 다 괜찮아 그냥 이대로도`];
            hookLong   = [`${t} 다 괜찮아 그냥 이대로도 충분하다는 거 이제 알아`];
        } else if (isSenior) {
            hookShort  = [`${t} 감사해요`];
            hookMedium = [`${t} 감사해요 이 모든 순간이`];
            hookLong   = [`${t} 감사해요 이 모든 순간 함께해줘서 진심으로`];
        } else if (isFormal && !isSenior) {
            hookShort  = [`${t} 괜찮아요`];
            hookMedium = [`${t} 괜찮아요 여기까지 왔으니까`];
            hookLong   = [`${t} 괜찮아요 여기까지 온 것만으로도 이미 충분해요`];
        } else {
            hookShort  = [t];
            hookMedium = [`${t} 이 노래가 네게 닿기를`];
            hookLong   = [`${t} 이 노래가 네게 닿기를 이 마음이 전해지기를`];
        }

        // ── 연령층별 Outro ──
        let outroShort, outroMedium, outroLong;
        if (isYoung) {
            outroShort  = [t, '잊지 마', '또 봐 우리'];
            outroMedium = [`${t} 이 감정 절대 잊지 마`, '언제나 응원할게 완전 진심으로'];
            outroLong   = [`${t} 이 순간 이 감정 영원히 기억해줘 나도 절대 안 잊을게`];
        } else if (isMid) {
            outroShort  = [t, '다 잘될 거야', '수고했어'];
            outroMedium = [`${t} 오늘 하루도 수고했어`, '천천히 가도 괜찮아 지금 이대로도'];
            outroLong   = [`${t} 오늘 하루도 정말 수고했어 내일은 조금 더 나아질 거야`];
        } else if (isSenior) {
            outroShort  = [t, '건강하세요', '감사합니다'];
            outroMedium = [`${t} 건강하게 오래오래 함께해요`, '언제나 곁에 있을게요 감사해요'];
            outroLong   = [`${t} 건강하고 행복하게 오래오래 함께해요 정말 감사합니다`];
        } else if (isFormal && !isSenior) {
            outroShort  = [t, '잘 하셨어요', '고마워요'];
            outroMedium = [`${t} 여기까지 와주셔서 감사해요`, '앞으로도 잘 부탁드려요 함께해요'];
            outroLong   = [`${t} 이 노래가 조금이나마 위로가 됐으면 좋겠어요 고마워요`];
        } else {
            outroShort  = [t, '기억할게', '항상 여기'];
            outroMedium = [`${t} 이 멜로디가 남아있을게`, `${feeling}이 담긴 이 노래 기억해줘`];
            outroLong   = [`${t} 이 노래와 함께했던 순간을 오래오래 기억해줘 나도 기억할게`];
        }
        ...
        return {
            ...
            'Post-Chorus': { short: ['오 오 오'], medium: ['오 오 오 예'], long: ['나 나 나 나 나'] },
            'Ad-lib':       { short: ['예'], medium: ['좋아 예'], long: ['한 번 더 예'] },
            Outro: {
                short:  outroShort,
                medium: outroMedium,
                long:   outroLong
