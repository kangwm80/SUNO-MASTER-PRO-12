...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function buildStoryPool(gen, place, sitText) {
        const situation = sitText || place;
        const pools = {
            '10대': [
                `${situation}. 평소라면 핸드폰만 봤을 텐데, 오늘은 그냥 주변을 둘러봤습니다. ${place}에 이렇게 다양한 사람들이 있다는 걸 처음 안 것 같습니다. 이어폰을 빼고 그 공간의 소리를 들어보니, 세상이 생각보다 시끄럽지 않습니다. 이 잠깐의 고요함 속에서 오늘의 내가 조금 더 선명하게 보입니다.`,
                `${situation}. 시험 때문에 머리가 터질 것 같았는데, ${place}에 있으니까 잠깐 숨이 쉬어집니다. 부모님은 열심히 하라고 하고, 선생님은 집중하라고 하는데, 지금 이 순간만큼은 아무도 나한테 뭔가를 요구하지 않습니다. 그냥 내가 나로 있을 수 있는 시간. 이 짧은 여유가 다시 책상에 앉을 힘을 만들어줍니다.`,
                `${situation}. 옆에 있는 친구한테 이 기분을 말하려다가 그냥 뒀습니다. 말로 설명하면 이상하게 들릴 것 같아서요. ${place}에서 나란히 앉아 각자 다른 걸 하면서 같이 있는 이 시간이, 어른이 돼서도 기억날 것 같은 예감이 듭니다. 말 없이도 통하는 친구가 있다는 것, 이미 행운입니다.`,
                `${situation}. 좋아하는 사람이 생겼습니다. ${place}에 오면 혹시 그 사람도 있을까 두리번거리는 나 자신이 좀 웃깁니다. 없어도 실망하지 않으려고 준비하고 왔는데, 막상 없으니까 역시 살짝 아쉽습니다. 이런 마음이 드는 게 좋다는 건지 불편하다는 건지 모르겠지만, 설레는 건 확실합니다.`,
                `${situation}. 어른들은 지금이 가장 좋은 시절이라고 하는데, 그 말이 왜 위로가 안 됐는지 몰랐습니다. 그런데 ${place}에서 이 순간을 즐기다 보니, 어쩌면 '지금'을 그냥 살면 되는 거겠구나 싶었습니다. 나중에 어른이 되면 이 시간을 그리워하겠죠. 그러니까 지금 잘 느껴두어야겠습니다.`,
                `${situation}. 친구 문제로 복잡한 하루였습니다. 머릿속이 시끄러웠는데 ${place}에 오니 잠깐 그 소리가 조용해졌습니다. 사람 관계가 이렇게 복잡한 줄 몰랐는데, 어른들도 이런 기분을 느끼는지 궁금합니다. 복잡한 것들을 잠시 내려놓고, 여기서 그냥 숨을 고릅니다.`,
                `${situation}. 핸드폰을 내려놓고 잠깐 아무 생각도 안 하기로 했습니다. 10분도 못 버틸 줄 알았는데 생각보다 괜찮습니다. ${place}의 공기, 소리, 풍경이 보이기 시작했습니다. 지금 이 순간, 생각보다 충분합니다.`,
                `${situation}. 요즘 내가 뭘 좋아하는지 잘 모르겠었는데, ${place}에서 이 기분을 느끼면서 '아, 이건 진짜 좋다'는 걸 알았습니다. 가짜가 아닌 진짜 기분. 이런 순간들을 더 많이 찾아다녀야겠다고 마음먹었습니다.`,
                `${situation}. 집에서 나오기 싫었는데, 나와서 다행입니다. 방에만 있으면 생각이 너무 많아지거든요. ${place}에서 주변 사람들을 보다 보니, 다들 자기 생각에 빠져있는 것 같고, 나만 이상한 게 아니라는 게 좀 위로됩니다.`,
                `${situation}. 오늘 이 기분을 일기에 적어두고 싶습니다. 나중에 어른이 됐을 때, 이 시간이 얼마나 소중했는지 기억하고 싶어서요. ${place}에서 보낸 이 오후가, 언젠가 가장 그리워지는 기억 중 하나가 될 것 같습니다.`
            ],
            '2030세대': [
                `${situation}. 번아웃이 와서 뭘 해도 의욕이 없던 시기였습니다. ${place}에서 아무 생각 없이 이 시간을 보내는 게 처음엔 죄책감이 들었습니다. 하지만 천천히, 이게 충전이라는 걸 받아들였습니다. 아무것도 생산하지 않는 이 시간이 결국 가장 생산적인 시간이라는 것을 배웠습니다.`,
                `${situation}. 회사 메신저 알림을 무음으로 해놓고 ${place}에 앉았습니다. 처음 30분은 손이 계속 핸드폰으로 갔는데, 1시간쯤 지나니 드디어 놓을 수 있었습니다. 업무에서 벗어나 나를 되찾는 시간. 워라밸의 진짜 의미를 이 순간에야 이해했습니다.`,
                `${situation}. 혼자가 외롭다고만 생각했는데, ${place}에서 혼자만의 시간을 가지다 보니 혼자인 것과 외로운 건 다르다는 걸 알게 됐습니다. 나 자신과 대화하는 법을 잊고 살았습니다. 타인의 시선과 SNS의 비교들에서 잠시 벗어나 나 자신에게 집중하는 이 시간이 지금 가장 필요한 것이었습니다.`,
                `${situation}. 친구한테 '나 오늘 ${place}에 있어'라고 카톡을 보냈습니다. '혼자?' '응.' '부럽다.' 요즘 혼자만의 시간이 사치가 된 세대입니다. 이런 여유 시간이 생겼다는 게 감사합니다. 이 시간을 낭비하지 않겠다고 다짐하며, 그냥 아무것도 안 하기로 했습니다. 그게 제일 잘 쉬는 방법이니까요.`,
                `${situation}. 20대의 나는 항상 '다음 단계'를 생각했습니다. 취업, 이직, 결혼, 집. 근데 ${place}에서 이 순간을 느끼면서, 도착점이 아니라 과정에 있다는 걸 처음으로 편하게 받아들였습니다. 지금 여기, 이 순간이 목적지라는 것. 삶은 준비하는 게 아니라 살아가는 거라는 당연한 말이 오늘은 다르게 들립니다.`,
                `${situation}. 연애가 끝난 지 석 달이 됐습니다. ${place}에 혼자 오는 게 처음엔 어색했지만, 오늘은 이상하게 괜찮습니다. 이별이 나에게 돌려준 것들이 있는지 모릅니다. 나 혼자만의 취향, 나 혼자만의 시간, 나 혼자만의 이 공간. 다시 시작할 준비가 조금씩 되고 있습니다.`,
                `${situation}. SNS에는 집 산 친구, 승진한 동료, 아이 낳은 동기들의 소식이 가득합니다. ${place}에서 핸드폰을 내려놓고 주위를 둘러보니, 다들 그냥 자기 일상을 살고 있습니다. 비교는 내가 선택하는 거였습니다. 오늘은 그 선택을 하지 않겠습니다.`,
                `${situation}. 요즘 내가 좋아하는 게 뭔지 모르겠었는데, ${place}에서 잊고 있던 것들이 하나둘 떠올랐습니다. 예전에 좋아했던 음악, 읽다 멈춘 책, 가고 싶었던 전시. 바쁘다는 핑계로 미뤄온 나 자신으로의 귀환을 시작할 때가 됐습니다.`,
                `${situation}. 부모님께 전화를 오래 못 한 것이 생각났습니다. ${place}에서 이렇게 여유를 부리면서, 그 시간에 전화 한 통 드리면 될 텐데 하는 생각. 오늘 집에 가는 길에는 반드시 전화를 드려야겠습니다.`,
                `${situation}. 이 순간을 기록해두고 싶어서 사진을 찍으려다 그냥 뒀습니다. ${place}에서의 이 시간은 '좋아요'가 필요 없는, 나만 알고 있는 행복입니다. 이런 비공개 순간들이 쌓여 진짜 삶이 됩니다.`
            ],
            '5060세대': [
                `${situation}. 30년 넘게 남들을 위해 살았습니다. 가족을 위해, 회사를 위해. ${place}에서 이렇게 나 자신을 위한 시간을 갖는 게 처음엔 어색했지만, 이제 압니다. 나를 채워야 남에게도 줄 수 있다는 것을. 이 여유로운 시간이 이기적인 것이 아니라 오히려 필요한 것이었다는 걸 알게 됐습니다.`,
                `${situation}. 자녀들이 독립하고 집이 갑자기 커진 것 같았습니다. 처음엔 그 빈자리가 허전했는데, ${place}에서 이렇게 시간을 보내다 보니 내 삶에도 자리가 생겼다는 걸 알게 됐습니다. 부모 이전에, 나라는 사람으로 살아볼 두 번째 기회입니다.`,
                `${situation}. 배우자와 함께 이 공간에 왔습니다. 30년을 함께 살아왔는데 아직도 배울 게 있습니다. ${place}에서 배우자가 웃는 걸 보면서, 이 사람이 여전히 좋다는 걸 새삼 확인했습니다. 익숙함이 권태가 아니라 편안함이 되는 경지에 이른 것 같습니다.`,
                `${situation}. 퇴직 후 처음으로 평일 낮에 ${place}에 왔습니다. 주변을 보니 다들 어디 급히 가는 것 같은데 나만 여유롭습니다. 바쁜 것이 정체성이었던 사람에게 여유는 낯선 영토입니다. 하지만 이 영토를 천천히 탐험해볼 작정입니다. 제2의 인생이 여기서 시작됩니다.`,
                `${situation}. 건강 검진에서 들은 말들이 머릿속을 맴돌다가 ${place}에 왔습니다. 쉬어야 하는데 쉬는 법을 모르는 세대. ${place}에서 이렇게 앉아있는 것도 연습이 필요하다는 것을 알게 됐습니다. 내 몸을 아끼는 것이 이제 가장 중요한 일입니다.`,
                `${situation}. 부모님이 이 나이에 어떻게 사셨는지 갑자기 궁금해졌습니다. ${place}에서 비슷한 또래의 사람들을 보다가, 우리 부모님도 이런 여유를 즐기셨을까 하는 생각이 들었습니다. 내가 그 나이가 되니 비로소 보이는 것들이 있습니다. 돌아가신 분들께 못다 한 이야기가 마음속에서 흐릅니다.`,
                `${situation}. 오래된 친구에게 연락이 왔습니다. '요즘 어때?' 이 짧은 문장에 많은 것이 담겨있습니다. ${place}에서 그 연락을 받고 오랜만에 웃었습니다. 오래된 인연의 소중함을 압니다. 이번 주말에는 꼭 만나야겠다고 마음먹었습니다.`,
                `${situation}. 젊었을 때 꿈꿨지만 하지 못한 것들이 있습니다. ${place}에서 이렇게 한가로이 앉아있다 보니, 그 꿈들이 떠올랐습니다. 지금 시작해도 괜찮다는 이야기를 들었습니다. 아직 남아있는 시간을 '늦은 시간'이 아니라 '새로운 시작의 시간'으로 보기로 했습니다.`,
                `${situation}. 손주 사진을 보며 혼자 미소 짓는 시간입니다. ${place}에서 이런 여유를 즐길 수 있다는 것이, 결국 우리 세대가 열심히 살아온 결과라는 생각이 듭니다. 고생했습니다, 나 자신에게. 남은 시간을 이렇게 잘 살아가는 것이 최선의 보답입니다.`,
                `${situation}. 이 나이에 무언가를 '처음' 해본다는 게 낯설었습니다. ${place}에서 이런 경험을 하면서, 아직 처음이 남아있다는 것이 얼마나 설레는 일인지 알게 됐습니다. 몸은 늙어도 마음의 나이는 내가 정하는 것이라는 말이 이제 피부로 느껴집니다.`
            ],
            '시니어세대': [
                `${situation}. 이 나이에 이런 여유가 생길 줄 몰랐습니다. ${place}에서 창밖을 바라보며 지나온 세월을 떠올립니다. 젊은 날의 바쁨도, 중년의 치열함도, 모두 이 여유로운 오후를 위한 준비였다는 생각이 듭니다. 감사합니다, 이 시간이 주어져서.`,
                `${situation}. 오래 전 배우자와 함께 이런 시간을 즐겼던 기억이 납니다. 지금은 혼자이지만 그 기억이 따뜻한 동반자가 되어줍니다. ${place}에서 그때의 두 사람을 상상합니다. 그리움은 아픈 감정이기도 하지만, 그만큼 사랑했다는 증거이기도 합니다.`,
                `${situation}. 손주가 보고 싶습니다. ${place}에서 어린 아이들을 보면 자연스럽게 손주 생각이 납니다. 다음에 손주가 오면 이 좋은 곳을 함께 와야겠다고 생각합니다. 아이들의 웃음소리가 세상에서 가장 좋은 음악이라는 걸 이 나이에야 완전히 알게 됐습니다.`,
                `${situation}. 평생 일하고 아이 키우느라 내 자신을 돌볼 여유가 없었습니다. ${place}에서 이렇게 조용히 앉아있는 시간이 '나'를 만나는 시간 같습니다. 평생 '엄마'로, '아내'로, '할머니'로 살았는데, 오늘 잠깐은 그냥 나 자신으로 있고 싶습니다.`,
                `${situation}. 같은 또래 친구를 만나면 서로의 건강 이야기가 빠지지 않습니다. ${place}에서 이렇게 건강하게 이 시간을 즐길 수 있다는 것이 새삼 감사합니다. 하루하루가 선물입니다. 오늘이라는 선물을 잘 받겠습니다.`,
                `${situation}. 젊은 사람들이 바쁘게 지나가는 걸 보면서 예전의 나를 봅니다. ${place}에서 여유롭게 앉아있는 이 시간은 그냥 얻어진 게 아닙니다. 평생을 부지런히 살아온 끝에 찾아온 선물입니다. 당당하게 앉아있겠습니다.`,
                `${situation}. 나이가 들수록 작은 것에서 행복을 찾게 됩니다. ${place}에서 느끼는 이 소소한 즐거움이 예전 같으면 대수롭지 않게 여겼을 것들입니다. 이 한 잔의 여유, 이 조용한 시간, 이 따뜻한 햇살이 충분합니다. 충분함을 아는 것이 지혜임을 이제야 알겠습니다.`,
                `${situation}. 평생을 살아오면서 가장 후회되는 것은 '더 많이 쉬지 못한 것'입니다. ${place}에서 이렇게 쉬면서 그 생각을 합니다. 하지만 후회는 과거를 위한 것이고, 지금 이 순간은 현재를 위한 것입니다. 지금부터라도 잘 쉬고 잘 즐기겠습니다. 늦지 않았습니다.`,
                `${situation}. 자녀들이 걱정할까봐 내 힘든 것을 말 못 하는 나이가 됐습니다. ${place}에서 혼자 이렇게 앉아있으면, 말하지 못한 마음들이 조금씩 풀립니다. 혼자만의 시간이 일종의 치유가 됩니다.`,
                `${situation}. 고요함이 이렇게 좋은 것인 줄 예전엔 몰랐습니다. ${place}에서 소란함 없이 그냥 있을 수 있다는 것. 인생의 모든 계절이 각자의 아름다움을 가지고 있다는 것. 지금 이 계절의 아름다움은 고요함과 여유입니다.`
            ]
        };
